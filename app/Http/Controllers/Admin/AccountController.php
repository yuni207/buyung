...
class AccountController extends Controller
{
    ...
    public function read(){
        $account = DB::table('users')->where('username','!=','admin')->orderBy('id','DESC')->get();
    ...
    public function reset($id){
        $defaultPassword = 'Buyung2026'; 
        DB::table('users')  
            ->where('id', $id)
            ->update([
            'password' => bcrypt($defaultPassword)]);

        return redirect('/admin/account')->with("success","Password Berhasil Direset ! | Password Default : {$defaultPassword}");
    }
}
